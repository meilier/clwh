...
empty_dict = torch.load(os.getcwd()+"/mnist-6000-1000-base.pth")

# initialize empty_dict to 0
empty_dict['fc.0.weight'] = np.zeros((784,400),dtype=float)
empty_dict['fc.0.bias'] = np.zeros((400,),dtype=float)
empty_dict['fc.2.weight'] = np.zeros((400,200),dtype=float)
empty_dict['fc.2.bias'] = np.zeros((200,),dtype=float)
empty_dict['fc.4.weight'] = np.zeros((200,100),dtype=float)
empty_dict['fc.4.bias'] = np.zeros((100,),dtype=float)
empty_dict['fc.6.weight'] = np.zeros((100,10),dtype=float)
empty_dict['fc.6.bias'] = np.zeros((10,),dtype=float)
    ...
    for i in range(len(diff)):
        if i < needed:
            if max_index[i] <= 313599:
                ...
                empty_dict['fc.0.weight'][row][column] = new['fc.0.weight'][row][column] - old['fc.0.weight'][row][column] 
            elif max_index[i] >= 313600 and max_index[i] <= 313999 :
                pos = max_index[i] - 313600
                empty_dict['fc.0.bias'][pos] = new['fc.0.bias'][pos] - old['fc.0.bias'][pos]
            elif max_index[i] >= 314000 and max_index[i] <= 393999:
                tmp = max_index[i] - 314000
                row = math.floor(tmp / 400)
                column = tmp % 400
                empty_dict['fc.2.weight'][row][column] =  new['fc.2.weight'][row][column] - old['fc.2.weight'][row][column]
            elif max_index[i] >= 394000 and max_index[i] <= 394199:
                pos = max_index[i] - 394000
                empty_dict['fc.2.bias'][pos] =  new['fc.2.bias'][pos] - old['fc.2.bias'][pos]
            elif max_index[i] >=394200 and max_index[i] <= 414199:
                tmp = max_index[i] - 394200
                row = math.floor(tmp / 200)
                column = tmp % 200
                empty_dict['fc.4.weight'][row][column] =  new['fc.4.weight'][row][column] - old['fc.4.weight'][row][column]
            elif max_index[i] >= 414200 and max_index[i] <= 414299:
                pos = max_index[i] - 414200
                empty_dict['fc.4.bias'][pos] =  new['fc.4.bias'][pos] - old['fc.4.bias'][pos] 
            elif max_index[i] >= 414300 and max_index[i] <= 415299:
                tmp = max_index[i] - 414300
                row = math.floor(tmp / 100)
                column = tmp % 100
                empty_dict['fc.6.weight'][row][column] = new['fc.6.weight'][row][column] - old['fc.6.weight'][row][column] 
            elif max_index[i] >= 415300 and max_index[i] <= 415309:
                pos = max_index[i] - 415300
                empty_dict['fc.4.bias'][pos] =  new['fc.6.bias'][pos] - old['fc.6.bias'][pos]
        else:
            break
    return empty_dict
...
for i in range(120):
    for j in range(10):
        ...
        # put empty_dict to chain
        partial_max_changed(tmp_ps_dict,current_dict, 0.1)
