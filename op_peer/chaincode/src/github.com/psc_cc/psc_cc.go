...
func (s *SmartContract) Init(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	fmt.Println("Parameter Service Init")
...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if function == "addGradientData" {
		return s.addGradientData(APIstub, args)
	} else if function == "getGradientData" {
		return s.getGradientData(APIstub, args)
	}
...
func (s *SmartContract) getParameter(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments-g. Expecting 1")
	}
...
// add gradient data
func (s *SmartContract) addGradientData(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	argslen := strconv.Itoa(len(args))
	fmt.Println("args len is " + argslen)
	if len(args) != 2 {
		return shim.Error("Incorrect number of arguments-a. Expecting 2")
	}
	gdAsBytes, _ := json.Marshal(args[1])
...
func (s *SmartContract) getGradientData(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	fmt.Println("args len is " + argslen)
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments-gg. Expecting 1")
	}
